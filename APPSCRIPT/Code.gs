...
function setupSheets() {
  ...
  // Setup Pengaturan (horizontal layout)
...
function setupTransaksiSheet(sheet) {
  const headers = ["ID", "Tanggal", "Proyek", "Kategori", "Jenis", "Deskripsi", "Nominal", "Metode", "PIC", "Catatan", "Bukti", "Timestamp", "User"];
  ...
  sheet.getRange("G:G").setNumberFormat("#,##0"); // Geser dari F ke G
  sheet.getRange("L:L").setNumberFormat("dd/MM/yyyy HH:mm"); // Geser dari K ke L
}
...
function setupPengaturanSheet(sheet) {
  ...
  // Header utama
  sheet.getRange("A1").setValue("PROYEK").setFontWeight("bold").setFontSize(12).setBackground("#3b82f6").setFontColor("#ffffff");
  sheet.getRange("F1").setValue("KATEGORI").setFontWeight("bold").setFontSize(12).setBackground("#10b981").setFontColor("#ffffff");
  sheet.getRange("I1").setValue("METODE").setFontWeight("bold").setFontSize(12).setBackground("#f59e0b").setFontColor("#ffffff");
  sheet.getRange("L1").setValue("PIC").setFontWeight("bold").setFontSize(12).setBackground("#8b5cf6").setFontColor("#ffffff");
  ...
  // Kolom separator
  sheet.setColumnWidth(5, 20);  // E - separator
  sheet.setColumnWidth(8, 20);  // H - separator
  sheet.setColumnWidth(11, 20); // K - separator
...
function setupSistemSheet(sheet) {
  ...
  sheet.clear();
  sheet.getRange("A1:C1").setValues([["Setting", "Value", "Keterangan"]])
    .setFontWeight("bold").setBackground("#1e293b").setFontColor("#ffffff");
...
function buildOptionData(ss) {
  ...
  // Read Proyek (kolom A-D, mulai baris 3)
  const proyekList = [];
  ...
  // Read Kategori (kolom F-G, mulai baris 3)
  const kategoriList = [];
  for (let i = 2; i < data.length; i++) {
  ...
  // Read Metode (kolom I-J, mulai baris 3)
  const metodeList = [];
  for (let i = 2; i < data.length; i++) {
  ...
  // Read PIC (kolom L-M, mulai baris 3)
  const picList = [];
  for (let i = 2; i < data.length; i++) {
...
function getHistoryData(ss, limit, offset, filterProyek, filterKategori) {
  ...
  let filtered = rows.map(row => ({
    ...
    jenis: String(row[4] || "Pengeluaran"), // <-- Tambahan kolom Jenis (Index 4)
    deskripsi: String(row[5] || ""),        // Geser jadi 5
    nominal: sanitizeNumber(row[6]),        // Geser jadi 6
    metode: String(row[7] || ""),           // Geser jadi 7
    pic: String(row[8] || ""),              // Geser jadi 8
    catatan: String(row[9] || ""),          // Geser jadi 9
    bukti: String(row[10] || ""),           // Geser jadi 10
    timestamp: row[11] ? new Date(row[11]).toISOString() : "", // Geser jadi 11
    user: String(row[12] || "")             // Geser jadi 12
  }));
...
function validatePayload(ss, payload, settings) {
  const required = ["tanggal", "proyek", "kategori", "nominal", "metode", "pic", "deskripsi", "user_input"];
...
function refreshDashboard() {
  ...
  for (const row of rows) {
    const nominal = sanitizeNumber(row[6]); // Geser dari 5 jadi 6
    ...
    const jenis = String(row[4] || "Pengeluaran"); // Ambil data jenis
    const proyek = String(row[2] || "Lainnya");
    const metode = String(row[7] || "Lainnya"); // Geser dari 6 jadi 7
    
    // (Biarkan logika perhitungannya di bawahnya tetap sama dulu untuk sekarang, 
    // nanti kita rombak dashboardnya khusus untuk menghitung Cash Flow)
  ...
  // Header
  dashboard.getRange("A1:H1").merge().setValue("DASHBOARD KEUANGAN")
    .setFontSize(18).setFontWeight("bold").setBackground("#1e293b").setFontColor("#ffffff")
    .setHorizontalAlignment("center");
  dashboard.getRange("A2:H2").merge().setValue("CV Akbar Dharma Karya • Update: " + lastUpdate)
    .setFontSize(10).setBackground("#334155").setFontColor("#94a3b8").setHorizontalAlignment("center");
  ...
  dashboard.getRange("B7").setValue(rows.length > 0 ? Math.round(totalNominal / rows.length) : 0).setNumberFormat("Rp #,##0");
  
  // Tabel per Kategori
  dashboard.getRange("D4").setValue("Per Kategori").setFontWeight("bold").setBackground("#3b82f6").setFontColor("#ffffff");
  let rowIdx = 5;
  ...
  // Tabel per Proyek
  dashboard.getRange("G4").setValue("Per Proyek").setFontWeight("bold").setBackground("#10b981").setFontColor("#ffffff");
  rowIdx = 5;
...
function processProofUpload(payload, settings, id) {
  ...
  if (!payload.bukti || !payload.bukti.includes("base64")) return "";
  
  try {
    const folder = DriveApp.getFolderById(FOLDER_ID);
    const parts = payload.bukti.split(",");
    const data = parts[1] || payload.bukti;
    const mime = parts[0].split(";")[0].replace("data:", "") || "image/jpeg";
    const ext = mime.includes("png") ? "png" : mime.includes("webp") ? "webp" : "jpg";
    const filename = "Bukti_" + id + "." + ext;
    
    // File berhasil dimasukkan ke drive
    const blob = Utilities.newBlob(Utilities.base64Decode(data), mime, filename);
    const file = folder.createFile(blob);
    
    // Kita paksakan script untuk terus berjalan (menyimpan link ke Sheet)
    // Walaupun di beberapa akun google workspace, mencoba 'setSharing' terkadang tidak dizinkan.
    try {
      file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
    } catch (sharingError) {
      // Jika setSharing dilarang oleh admin sistem google/error, abaikan saja dan lanjut
      Logger.log("Sharing error: " + sharingError.message);
    }
    
    // Tetap kembalikan Link URL file agar tersimpan di spreadsheet 
    return file.getUrl();
